#!/usr/bin/env python3
"""PaleONT unit tests — every CQ must return rows, every NCQ must return none.

Runs each query twice: on the raw graph, and on the graph closed under RDFS
entailment (owlrl). Queries that need a reasoner are visible as the difference.
"""
import sys, glob, os
...
# CQ10 needs a second specimen to be arithmetically possible; flagged, not failed.
